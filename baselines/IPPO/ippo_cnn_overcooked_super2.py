...
def get_rollout(params, config):
    ...
    while not done:
        ...
        shaped_rewards_seq.append(info["shaped_reward"]['agent_0'])        

    return state_seq, shaped_rewards_seq
...
@hydra.main(version_base=None, config_path="config", config_name="ippo_cnn_overcooked_super2")
def main(config):
    if config["TUNE"]:
        tune(config)
    else:
